...
class IndentCommissionReportWizard(models.TransientModel):
    _name = 'indent.commission.report.wizard'
    ...
    def get_report(self):
        ...
        report = 'INDENT COMMISSION REPORT DETAIL - FROM ' + str(self.from_date) + ' to ' + str(self.to_date)
        workbook = xlsxwriter.Workbook(report)

        # Create worksheet 1
        worksheet = workbook.add_worksheet('INDENT COMMISSION DETAIL')
        ...
        worksheet.write(row + 4, col, "INDENT COMMISSION REPORT DETAIL - FROM " + str(self.from_date) + ' TO ' + str(self.to_date), heading_2)
        ...
        worksheet.write(row, col + 2, "Date of income Recognition", font_center_bold)
        worksheet.write(row, col + 3, "Supplier", font_center_bold)
        worksheet.write(row, col + 4, "Customer", font_center_bold)
        worksheet.write(row, col + 5, "Product name", font_center_bold)
        worksheet.write(row, col + 6, "BL date", font_center_bold)
        worksheet.write(row, col + 7, "Credit period", font_center_bold)
        worksheet.write(row, col + 8, "Date of payment made by supplier", font_center_bold)
        worksheet.write(row, col + 9, "Currency type", font_center_bold)
        worksheet.write(row, col + 10, "Indent Value", font_center_bold)
        worksheet.write(row, col + 11, "Commission %", font_center_bold)
        worksheet.write(row, col + 12, "Commission Value (USD/EURO)", font_center_bold)
        worksheet.write(row, col + 13, "Commission Value (LKR)", font_center_bold)
    ...
    def download_report(self):
        ...
        values = {
            'name': 'INDENT COMMISSION REPORT ' + str(self.from_date) + ' to ' + str(self.to_date),
