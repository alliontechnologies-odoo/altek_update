...
class IndentCommissionSalesPersonWiseReportWizard(models.TransientModel):
    _name = 'indent.commission.sales.person.wise.report.wizard'
    ...
    def get_report(self):
        ...
        report = 'INDENT COMMISSION - SALES PERSON WISE  - FROM ' + str(self.from_date) + ' to ' + str(self.to_date)
        workbook = xlsxwriter.Workbook(report)

        # Create worksheet 1
        worksheet = workbook.add_worksheet('INDENT COMMISSION - SALES PERSON WISE')
        ...
        worksheet.write(row + 4, col, "INDENT COMMISSION - SALES PERSON WISE  - FROM " + str(self.from_date) + ' TO ' + str(self.to_date), heading_2)

        col = 0
        row = 6

        worksheet.write(row, col, "Indent income no", font_center_bold)
        worksheet.write(row, col + 1, "Month of Recognition", font_center_bold)
        worksheet.write(row, col + 2, "Supplier name", font_center_bold)
        worksheet.write(row, col + 3, "Customer name", font_center_bold)
    ...
    def download_report(self):
        ...
        values = {
            'name': 'INDENT COMMISSION - SALES PERSON WISE ' + str(self.from_date) + ' to ' + str(self.to_date),
