...
class IndentCommissionIndividualPerformanceReportWizard(models.TransientModel):
    _name = 'indent.commission.individual.performance.report.wizard'
    ...
    def get_report(self):
        ...
        report = 'INDENT COMMISSION - INDIVIDUAL PERFORMANCE - FROM ' + str(self.from_date) + ' to ' + str(self.to_date)
        workbook = xlsxwriter.Workbook(report)

        # Create worksheet 1
        worksheet = workbook.add_worksheet('INDENT COMMISSION - INDIVIDUAL PERFORMANCE')
        ...
        worksheet.write(row + 4, col, "INDENT COMMISSION - INDIVIDUAL PERFORMANCE - FROM " + str(self.from_date) + ' TO ' + str(self.to_date), heading_2)

        col = 0
        row = 6

        worksheet.write(row, col, "Sales person", font_center_bold)
        worksheet.write(row, col + 1, "Supplier name", font_center_bold)
        worksheet.write(row, col + 2, "Budgeted amount", font_center_bold)
        worksheet.write(row, col + 3, "Forecasted amount", font_center_bold)
        worksheet.write(row, col + 4, "Actual amount", font_center_bold)
        worksheet.write(row, col + 5, "%", font_center_bold)
    ...
    def download_report(self):
        ...
        values = {
            'name': 'INDENT COMMISSION - INDIVIDUAL PERFORMANCE ' + str(self.from_date) + ' to ' + str(self.to_date),
