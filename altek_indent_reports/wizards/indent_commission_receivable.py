...
class IndentCommissionReceivableReportWizard(models.TransientModel):
    _name = 'indent.commission.receivable.report.wizard'
    ...
    def get_report(self):
        ...
        report = 'INDENT COMMISSION - RECEIVABLE - FROM ' + str(self.from_date) + ' to ' + str(self.to_date)
        workbook = xlsxwriter.Workbook(report)

        # Create worksheet 1
        worksheet = workbook.add_worksheet('INDENT COMMISSION - RECEIVABLE')
        ...
        worksheet.write(row + 4, col, "INDENT COMMISSION - RECEIVABLE - FROM " + str(self.from_date) + ' TO ' + str(self.to_date), heading_2)
        ...
        worksheet.write(row, col + 3, "BL Date", font_center_bold)
        worksheet.write(row, col + 4, "Credit period", font_center_bold)
        worksheet.write(row, col + 5, "Date of payment made by supplier", font_center_bold)
        worksheet.write(row, col + 6, "Supplier", font_center_bold)
        worksheet.write(row, col + 7, "Customer", font_center_bold)
        worksheet.write(row, col + 8, "Product name", font_center_bold)
        worksheet.write(row, col + 9, "Commission Value", font_center_bold)
        worksheet.write(row, col + 10, " No of days from recognition", font_center_bold)
    ...
    def download_report(self):
        ...
        values = {
            'name': 'INDENT COMMISSION - RECEIVABLE ' + str(self.from_date) + ' to ' + str(self.to_date),
