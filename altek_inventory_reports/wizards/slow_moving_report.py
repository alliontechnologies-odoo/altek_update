...
class SlowMovingReportWizard(models.TransientModel):
    _name = 'slow.moving.report.wizard'
    ...
    def get_report(self):
        ...
        report = 'SLOW MOVING REPORT - OVER 90 DAYS - FROM ' + str(self.from_date) + ' to ' + str(self.to_date)
        workbook = xlsxwriter.Workbook(report)

        # Create worksheet 1
        worksheet = workbook.add_worksheet('SLOW MOVING REPORT')
        ...
        worksheet.write(row + 4, col, "SLOW MOVING REPORT - OVER 90 DAYS - FROM " + str(self.from_date) + ' TO ' + str(self.to_date), heading_2)
        ...
        worksheet.write(row, col + 2, "Quantity (KG.)", font_center_bold)
        worksheet.write(row, col + 3, "Value (RS.)", font_center_bold)
        worksheet.write(row, col + 4, "Expiry Date", font_center_bold)
    ...
    def download_report(self):
        ...
        values = {
            'name': 'SLOW MOVING REPORT - OVER 90 DAYS ' + str(self.from_date) + ' to ' + str(self.to_date),
