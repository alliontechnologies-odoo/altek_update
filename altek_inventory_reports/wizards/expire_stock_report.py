...
class ExpireStockReportWizard(models.TransientModel):
    _name = 'expire.stock.report.wizard'
    ...
    def get_report(self):
        ...
        report = 'EXPIRY STOCK  - FROM ' + str(self.from_date) + ' to ' + str(self.to_date)
        ...
        worksheet.write(row + 4, col, "EXPIRY STOCK - FROM " + str(self.from_date) + ' TO ' + str(self.to_date), heading_2)

        col = 0
        row = 6

        worksheet.write(row, col, "Category", font_center_bold)
        worksheet.write(row, col + 1, "Item Name", font_center_bold)
        worksheet.write(row, col + 2, "Quantity", font_center_bold)
        worksheet.write(row, col + 3, "Avg Cost", font_center_bold)
        worksheet.write(row, col + 4, "Item Value", font_center_bold)
        worksheet.write(row, col + 5, "Expiry Date", font_center_bold)
        worksheet.write(row, col + 6, "No. of Days to Expire", font_center_bold)
    ...
    def download_report(self):
        ...
        values = {
            'name': 'EXPIRY STOCK ' + str(self.from_date) + ' to ' + str(self.to_date),
