...
class SaleGPReportWizard(models.TransientModel):
    _name = 'sale.g.p.report.wizard'
    ...
    def get_report(self):
        ...
        report = 'SALES AND GP REPORT - FROM ' + str(self.from_date) + ' to ' + str(self.to_date)
        workbook = xlsxwriter.Workbook(report)

        # Create worksheet 1
        worksheet = workbook.add_worksheet('EXPIRY STOCK')
        ...
        worksheet.write(row + 4, col, "SALES AND GP REPORT - FROM " + str(self.from_date) + ' TO ' + str(self.to_date), heading_2)

        col = 0
        row = 6

        worksheet.write(row, col, "Category", font_center_bold)
        worksheet.write(row, col + 1, "Budget Sale", font_center_bold)
        worksheet.write(row, col + 2, "Forecasted Sale", font_center_bold)
        worksheet.write(row, col + 3, "Actual Sale to date", font_center_bold)
        worksheet.write(row, col + 4, "Budget GP", font_center_bold)
        worksheet.write(row, col + 5, "Forecasted GP", font_center_bold)
        worksheet.write(row, col + 6, "Actual GP to date", font_center_bold)
        worksheet.write(row, col + 7, "Actual GP % ", font_center_bold)
    ...
    def download_report(self):
        ...
        values = {
            'name': 'SALES AND GP REPORT ' + str(self.from_date) + ' to ' + str(self.to_date),
